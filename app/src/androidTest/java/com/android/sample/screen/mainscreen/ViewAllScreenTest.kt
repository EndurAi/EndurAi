...
class ViewAllScreenTest {
    ...
    fun setUp() {
        ...
        val yogaWorkouts = listOf(
            YogaWorkout("3", "Sunrise", "Perform morning yoga routine", false),
            YogaWorkout("4", "Relax", "Hold the pose for balance", false)
        )

        `when`(bodyWeightRepo.getDocuments(any(), any())).thenAnswer {
            it.getArgument<(List<BodyWeightWorkout>) -> Unit>(0)(bodyWeightWorkouts)
        }

        `when`(yogaRepo.getDocuments(any(), any())).thenAnswer {
