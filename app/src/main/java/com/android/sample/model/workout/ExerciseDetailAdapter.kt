...
class ExerciseDetailAdapter : JsonDeserializer<ExerciseDetail>, JsonSerializer<ExerciseDetail> {
    ...
    override fun deserialize(
        ...
        val jsonObject = json?.asJsonObject
            ?: throw JsonParseException("Invalid JSON for ExerciseDetail")

        return when (jsonObject.get("type").asString) {
            "RepetitionBased" -> ExerciseDetail.RepetitionBased(
                repetitions = jsonObject.get("repetitions").asInt
            )
            "TimeBased" -> ExerciseDetail.TimeBased(
