...
fun CalendarScreen(
    ...
    val coloredWorkoutsBody = workoutsBody.map { ColoredWorkout(it, PastelBlue, WorkoutType.BODY_WEIGHT) } // Rouge pastel
    val coloredWorkoutsYoga = workoutsYoga.map { ColoredWorkout(it, PastelRed, WorkoutType.YOGA) } // Bleu pastel
                    ...
                    OutlinedButton(
                        onClick = {
                            when    (selectedWorkout!!.type){
                                WorkoutType.BODY_WEIGHT -> bodyworkoutViewModel.deleteWorkoutById(selectedWorkout!!.workout.workoutId)
                                WorkoutType.YOGA -> yogaworkoutViewModel.deleteWorkoutById(selectedWorkout!!.workout.workoutId)
                                WorkoutType.RUNNING -> {}
                            }
                            showDialog = false
                        },
    ...
    val workoutsByDate = workouts.groupBy {
        it.workout.date.toLocalDate().toKotlinLocalDate()
    }
            ...
            LazyColumn(
                ...
                items(dates) { date ->
                    DaySection(
                        ...
                        onWorkoutClick = {workout -> selectedWorkout = workout
                            showDialog = true}
                    )
                }
            }
    ...
    Row(
        ...
        if (workouts.isEmpty()) {
            ...
            Column(
                ...
                workouts.sortedBy { it.workout.date }.forEach { workout ->
...
fun WorkoutItem(
    ...
    Card(
        ...
        colors = CardDefaults.cardColors(
            containerColor = coloredWorkout.backgroundColor
        ),
        shape = MaterialTheme.shapes.medium
    ) {
        Row(
