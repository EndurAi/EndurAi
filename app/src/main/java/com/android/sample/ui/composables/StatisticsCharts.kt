...
fun Charts(data: List<Double>, labelTitle: String) {
    ...
    LineChart(
        modifier = Modifier.height(150.dp).width(200.dp).testTag("Chart"),
        data =
        listOf(
            Line(
                label = labelTitle,
...
@Composable
fun PieChartWorkoutType(frequency: Map<WorkoutType, Double>) {
    val data =
        listOf(
            frequency.get(WorkoutType.BODY_WEIGHT)?.let {
                Pie(
                    label = stringResource(R.string.TitleTabBody),
                    data = it,
                    color = LegendBodyweight,
                    selectedColor = LegendBodyweight)
            },
            frequency.get(WorkoutType.YOGA)?.let {
                Pie(
                    label = stringResource(R.string.TitleTabYoga),
                    data = it,
                    color = LegendYoga,
                    selectedColor = LegendYoga)
            },
            frequency.get(WorkoutType.RUNNING)?.let {
                Pie(
                    label = stringResource(R.string.TitleTabRunning),
                    data = it,
                    color = LegendRunning,
